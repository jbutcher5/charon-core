...
use crate::models::{WCode, State, Token};
...
impl WParser for State {
    fn parser(&self, code: Vec<(LToken, Span)>, reference: &str) -> Vec<WCode> {
        ...
        let parse = |s: &str| self.parser(LToken::lexer(s).spanned().collect::<Vec<_>>(), s)[0].default_case.clone();
        for (token, span) in code {
            if let LToken::Newline = token {
                ...
                if let Some(func_reference) = FUNCTIONS.get(&func)  {
                    current_container.default_case.push(Token::Function(*func_reference))
                } else {
                    current_container.default_case.push(Token::Container(func))
                }
            } else if let LToken::FunctionLiteral(func) = token {
                if let Some(func_reference) = FUNCTIONS.get(&func)  {
                    current_container.default_case.push(Token::FunctionLiteral(*func_reference))
                } else {
                    current_container.default_case.push(Token::ContainerLiteral(func))
                }
            }
